...
namespace Azure.shovel
{
    ...
    public static class shovel
    {
        ...
        public static async Task TenantMdmOutToMdmIn(
            [ServiceBusTrigger("sendera", Connection = "tenant-out-connection")] Message[] input,
            ILogger log)
        {
            string tenantValue = Environment.GetEnvironmentVariable("tenant-name");
            log.LogInformation($"The tenantValue ist: {tenantValue}");
                ...
                try
                {
                    ...
                    if (message.UserProperties.TryGetValue("appid", out appId))
                    {
                        if (IsRouteAllowed(tenantValue, appId.ToString(), log))
                        {
                            ...
                            string targetQueueConnectionString = Environment.GetEnvironmentVariable(appId.ToString());

                            await ForwardToTargetQueue(message, targetQueueConnectionString, appId.ToString(), log);
        ...
        private static async Task ForwardToTargetQueue(Message message, string connectionString, string nameQueue, ILogger log, Func<Message, Message> factory = null)
        {
            ...
            QueueClient client = new QueueClient(connectionString, nameQueue);

            // send the message
            await client.SendAsync(forwardedMessage);
            Console.WriteLine($"Sent a single message to the queue: " + nameQueue);
